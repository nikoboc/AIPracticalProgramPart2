...
### 【覚え書き】題材として Tavily を採用し続ける理由（2026-09-22 検討）

Tavily は多くの受講者にとって初見のサービスであり、「単なる例なら受講者の身近なサービスにしたい」という観点で**国内でよく使われるサービスへの差し替えを検討した**。結論は **Tavily を維持**。理由と検討結果を残す。

**この回の題材に求める条件**（⑦⑧から逆算）

1. API キーが無料・クレジットカード不要・数分で発行できる（④の「会員証」の実演）。
2. n8n に専用ノードがない（⑦の主題は「専用ノードがないサービスと繋ぐ方法」）。
3. 公式ドキュメントに cURL の例があり、Import cURL がそのまま使える。
4. キーを Credential（Header Auth）に登録でき、追加のヘッダーが要らない。
5. AI エージェントの「調べる」ツールとして成立し、⑧のリサーチ担当サブエージェントにそのまま移植できる。
6. POST ＋ JSON ボディで、GET/POST・ダブルクォートの説明が活きる。

**検討した国内候補と、外れる条件**

| 候補 | 身近さ | 外れる条件 |
|---|---|---|
| ホットペッパーグルメサーチ API（リクルート WEB サービス） | ◎ メール登録だけでキー発行 | GET のみ／キーはクエリパラメータ `key=`（Query Auth になる）／公式に cURL 例がない → 3・4・6 |
| 楽天市場商品検索 API（Rakuten Developers） | ◎ | 2026 年の API 刷新で `accessKey` ヘッダーに加え **Origin / Referer ヘッダーが必須**。アプリ登録時のドメイン設定と 403 の切り分けが受講者に乗る → 4 |
| LINE Messaging API | ◎ 最も身近 | 「調べる」ではなく「送る」用途。⑧をリサーチ担当から通知担当へ再設計する必要があり、⑥の Gmail と役割が重なる。チャネル作成・userId 取得など設定手順も多い → 5 |
| Gemini API（Google AI Studio） | ○ ⑨で既出 | n8n に **Google Gemini の専用ノードがある** → 2 |
| Yahoo! JAPAN テキスト解析 Web API | ○ | 「テキスト解析」は受講者に効用が伝わりにくく、⑧の題材にならない → 5 |
| Slack / Notion / YouTube Data API / DeepL | ◎ | いずれも専用ノードあり。DeepL API Free はクレジットカード必須、YouTube は Google Cloud Console の設定が重い → 1・2 |
| Chatwork / kintone | △ 業務利用者のみ | Chatwork はボディがフォーム形式で JSON の説明が使えない。kintone は先にアプリを作る必要があり手順が重い → 6 |

**Tavily を維持する理由**

- 上の 6 条件をすべて満たす唯一の候補。Quickstart の cURL が POST・JSON・`Authorization: Bearer` の形で載っており、⑦の説明と Credential の手順が一切崩れない。
- 返るのが Web 検索の結果なので、⑧のリサーチ担当サブエージェントにそのまま繋がる。
- **初見のサービスであること自体が ⑦ のメッセージと一致する。** この回の主張は「知らないサービスでも、ドキュメントと cURL さえ見つければ繋げる」であり、受講者が知らないサービスで成功する体験のほうが裏づけになる。
- 受講者の負担の本体は API・cURL・ヘッダーという仕組み側にあり、サービス名ではない。

**負担を下げるための扱い**

- Tavily の説明は一言に固定する：「**AI 向けの検索エンジン。ChatGPT が Web を調べるときに裏で使うような部品**」。①または事前課題の案内で先に伝えておく。
- 身近さは題材の差し替えではなく、**「専用ノードがないサービスの例」のスライドに LINE やホットペッパーを並べる**、⑨の自由演習の候補としてホットペッパーグルメサーチ API を挙げる、という形で取り込む。

> 注意：上記の各サービスの仕様は 2026-09-22 時点の Web 検索で確認したもの（公式サイトの直接確認は未実施）。候補を再検討する際は、必ず各社の公式ドキュメントで当日の仕様を確認すること。
