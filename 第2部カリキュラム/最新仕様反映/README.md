...
| OpenAI 100 クレジット無料特典 | n8n から提供 | **現行の公開情報では確認できない**（要再確認） | — |
...
| 機能 | 内容 | 関連回 |
|---|---|---|
| **Gateway credits** | プロバイダのアカウント・API キーなしで AI モデルを動かせる（n8n 2.36 以降 / Cloud Starter・Pro） | ④ |
...
n8n は更新が非常に速い（2026年に入ってから 2.0 → 2.40）。以下は**特に変わりやすい／確定できなかった**項目。

| # | 確認すること | 理由 |
|---|---|---|
| 1 | **無料トライアルのアカウントで Gateway credits が使えるか／初期残高はいくらか** | ドキュメントは「Cloud Starter / Pro」と記載。トライアルでの扱いが明記されていない | 
| 2 | **n8n から OpenAI 100 クレジットの特典が今も提供されているか** | 現行の公開情報では確認できず。Gateway credits に統合された可能性が高い |
| 3 | **Overview ページのメニュー配置（タブかサイドバーか）** | バージョンとプランで変わる。収録当日の画面で確認する |
| 4 | **オートセーブの提供状態**（2.4 時点では Beta と告知されていた） | 公式ドキュメントは現在 Beta の注記なしで記載しているが、念のため |
| 5 | **⑨の演習用ワークフローの配布 URL** | 元動画では「こちらが指定する URL」とのみ言及され、文字起こしに URL が残っていない。**再収録時に確定が必要**（仕様変更ではない） |
| 6 | **AI Agent ノードのバージョン**（現行 v3） | v1 は n8n 3.0 で削除予定。テンプレート流用時に影響 |
| 7 | **Starter / Pro プランで1アカウントが複数インスタンスを持てるか** | 公式ドキュメントに明示なし。インスタンス名の変更に新規アカウント作成を求めている記述から実質1対1と読めるが、断定できない |
...
- ⑨の演習用ワークフローのダウンロード URL が動画中で明示されていない。
